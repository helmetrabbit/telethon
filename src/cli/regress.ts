...
function main(): void {
  ...
  for (const tc of suite.cases) {
    ...
    if (result.roleClaim) {
      const status = computeStatus(result.roleClaim.probability, input.totalMsgCount, result.roleClaim.evidence);
      actuals.push({ predicate: 'has_role', value: result.roleClaim.label, status });
    }
    ...
    if (tc.expectedGated && (result.roleClaim || result.intentClaim)) {
