...
// ── Temporal Bias ──────────────────────────────────────
const REFERENCE_DATE = new Date('2026-02-07'); // "Now" for this context
const HALF_LIFE_DAYS = 180; // 6 months

export function calculateTimeDecay(sentAtStr: string): number {
  const sentAt = new Date(sentAtStr);
  const diffDays = (REFERENCE_DATE.getTime() - sentAt.getTime()) / (1000 * 3600 * 24);
  if (diffDays < 0) return 1.0; // Future safety
...
export function scoreUser(input: UserInferenceInput, config: InferenceConfig): UserInferenceResult {
  ...
  const msgRoleHits = new Map<string, number>(); // tag → weighted_score
  const msgRoleCounts = new Map<string, number>(); // tag → raw_count (for logging)
  const msgIntentHits = new Map<string, number>();
  const msgIntentCounts = new Map<string, number>(); // tag → raw_count
  ...
  for (let mi = 0; mi < messageSample.length; mi++) {
    ...
    // Calculate temporal weight for this message (default 1.0 if no meta)
    const timeWeight = meta ? calculateTimeDecay(meta.sent_at) : 1.0;

    // Meme / Shitpost detection
    if (MEME_CONTENT_PATTERNS.some((p) => p.test(text))) {
        memeHitCount++;
    }
    ...
    for (const kw of MSG_ROLE_KEYWORDS) {
      ...
      if (m) {
        ...
        // Add time-weighted match
        msgRoleHits.set(key, (msgRoleHits.get(key) ?? 0) + timeWeight);
        // Track raw count for log-scaling logic
        msgRoleCounts.set(key, (msgRoleCounts.get(key) ?? 0) + 1);
        ...
        if (input.trace && meta) {
          messageHits.push({
            ...
            label: kw.label,
            label_type: 'role',
            weight: kw.weight * timeWeight, // Trace shows effective weight
          });
        }
      }
    }
    for (const kw of MSG_INTENT_KEYWORDS) {
      ...
      if (m) {
        const key = `${kw.label}:${kw.tag}`;
        msgIntentHits.set(key, (msgIntentHits.get(key) ?? 0) + timeWeight);
        msgIntentCounts.set(key, (msgIntentCounts.get(key) ?? 0) + 1);
  ...
  // Modified v0.6.0: Use time-weighted hits for score calculation, but log-scaling relies on
  // the 'density' of the signal. We apply log scaling to the aggregated time-weighted score
  // effectively treating '1 recent message' > '1 old message'.
  for (const [key, weightedScore] of msgRoleHits) {
    const [label, tag] = key.split(':') as [Role, string];
    const kw = MSG_ROLE_KEYWORDS.find((k) => k.label === label && k.tag === tag);
    if (!kw) continue;

    const rawCount = msgRoleCounts.get(key) ?? 1;
    // Log scaling logic: traditionally Math.log2(1 + count).
    // With temporal weights, 'weightedScore' is the sum of time_factors.
    // If I sent 10 messages long ago (factor 0.1), weightedScore = 1.0. 
    // If I sent 1 message today (factor 1.0), weightedScore = 1.0.
    // This seems fair.
    const finalWeight = kw.weight * Math.log2(1 + weightedScore);
    
    roleScores.set(label, (roleScores.get(label) ?? 0) + finalWeight);
    roleEvidence.get(label)!.push({
      evidence_type: 'message',
      evidence_ref: `msg:${tag}:n=${rawCount}:w=${weightedScore.toFixed(2)}`,
  ...
  if (hasBuilderTechMsg && !hasBuilderActionMsg && !hasBuilderBioOrDN) {
    ...
    // We recalculate this based on the weighted scores in the evidence
    const techContrib = roleEvidence.get('builder')!
      .filter((e) => e.evidence_ref.includes('msg:builder_tech'))
  ...
  for (const [key, weightedScore] of msgIntentHits) {
    const [label, tag] = key.split(':') as [Intent, string];
    const kw = MSG_INTENT_KEYWORDS.find((k) => k.label === label && k.tag === tag);
    if (!kw) continue;

    const rawCount = msgIntentCounts.get(key) ?? 1;
    const finalWeight = kw.weight * Math.log2(1 + weightedScore);
    
    intentScores.set(label, (intentScores.get(label) ?? 0) + finalWeight);
  ...
  // Meme / Shitpost override:
  // If user displays meme behavior or self-identifies as a shitposter, 
  // strongly penalize professional roles (founder, bd, investor).
  if (memeHitCount > 0) {
    const penalty = memeHitCount * 5.0; 
    const profRoles: Role[] = ['founder_exec', 'bd', 'investor_analyst'];
    for (const r of profRoles) {
        if ((roleScores.get(r) ?? 0) > 0) {
            roleScores.set(r, (roleScores.get(r) ?? 0) - penalty);
            roleEvidence.get(r)!.push({
                evidence_type: 'message',
                evidence_ref: `msg:meme_penalty:count=${memeHitCount}`,
                weight: -penalty
            });
        }
    }
  ...
  // Multi-label role support: iterate all candidates
  for (const roleCandidate of rankedRoles) {
    const nonMembershipEvidence = roleCandidate.evidence.filter(
      (e) => e.evidence_type !== 'membership',
    );
     
    let gated = false;
    if (nonMembershipEvidence.length < config.gating.minNonMembershipEvidence) {
      if (roleCandidate === topRole) {
        result.gatingNotes.push(
            `role:${roleCandidate.label} GATED — only ${nonMembershipEvidence.length} non-membership evidence (need ≥${config.gating.minNonMembershipEvidence})`
        );
      }
      gated = true;
    } else if (roleCandidate.probability < config.gating.minClaimConfidence) {
      if (roleCandidate === topRole) {
         result.gatingNotes.push(
            `role:${roleCandidate.label} GATED — confidence ${roleCandidate.probability.toFixed(3)} < threshold ${config.gating.minClaimConfidence}`
         );
      }
      gated = true;
    }
    ...
    if (!gated) {
      result.roleClaims.push(roleCandidate);
    }
  }
