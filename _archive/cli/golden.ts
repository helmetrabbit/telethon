...
async function main(): Promise<void> {
  ...
  for (const tc of suite.cases) {
    ...
    // Check role
    // For golden tests, we check if the EXPECTED role is present in the set of inferred roles.
    // If expected is NULL, we ensure NO roles were inferred.
    const actualRoles = result.roleClaims.map(r => r.label);
    const primaryRoleMatch = tc.expected_role 
      ? result.roleClaims.find(r => r.label === tc.expected_role)
      : null;

    if (tc.expected_role) {
      if (!primaryRoleMatch) {
         // Expected a role, but didn't find it
         // Use the highest probability role as the "actual" to show in error message, or just list them all
         const topRole = result.roleClaims[0]; 
         errors.push(`role: expected=${tc.expected_role}, got=${actualRoles.join(', ') || 'NONE'} (top_p=${topRole?.probability.toFixed(3) ?? '—'})`);
      }
    } else {
      // Expected NULL (no role)
      if (result.roleClaims.length > 0) {
        const topRole = result.roleClaims[0];
        errors.push(`role: expected=NONE, got=${actualRoles.join(', ')} (top_p=${topRole.probability.toFixed(3)})`);
      }
    }
    ...
    if (errors.length === 0) {
      passed++;
      const rolesDisplay = result.roleClaims.length > 0 
          ? result.roleClaims.map(r => `${r.label}(${r.probability.toFixed(2)})`).join(', ')
          : 'NONE';
      console.log(`  ✅ ${tc.id} → ${rolesDisplay}`);
    } else {
      failed++;
      const summary = `  ❌ ${tc.id}\n${errors.map(e => `     → ${e}`).join('\n')}`;
      console.log(summary);
