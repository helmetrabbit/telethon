...
async function main(): Promise<void> {
  ...
  for (const userId of uniqueIds) {
    ...
    for (const claim of allClaims) {
      ...
      if (claim.predicate === 'has_role') {
        const rc = traceResult.roleClaims.find(r => r.label === claim.object_value);
        if (rc) {
          finalScore = rc.score;
          probability = rc.probability;
          evidenceItems = rc.evidence.map(e => ({
    ...
    // Role claim trace
    const roleClaimsTrace = traceResult.roleClaims.map(rc => ({
      label: rc.label,
      raw_score: parseFloat(rc.score.toFixed(4)),
      probability: parseFloat(rc.probability.toFixed(6)),
      evidence: rc.evidence.map(e => ({
        source_type: e.evidence_type,
        evidence_ref: e.evidence_ref,
        pattern_id: e.evidence_ref.split(':')[1] ?? e.evidence_ref,
        weight: e.weight,
    ...
    traceProfiles.push({
      user_id: user.id,
      display_name: user.display_name,
      role_claims_trace: roleClaimsTrace,
