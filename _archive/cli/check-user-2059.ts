...
async function main() {
    const userId = 2059; // The specific user reported
    ...
    // 2. Check Messages (Ensure enough context exists)
    const msgRes = await db.query('SELECT count(*) as cnt FROM messages WHERE user_id = $1', [userId]);
    console.log(`   Messages: ${msgRes.rows[0].cnt}`);
