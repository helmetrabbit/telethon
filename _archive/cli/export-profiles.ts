...
interface ProfilePsychographic {
  model: string;
  tone: string | null;
...
interface UserProfile {
  ...
  claims: ProfileClaim[];
  psychographic: ProfilePsychographic | null;
}
...
async function buildProfile(user: UserRow, engineVersion: string): Promise<UserProfile> {
  ...
  const psychoRes = await db.query<{
    model_name: string; tone: string | null; professionalism: string | null;
    verbosity: string | null; responsiveness: string | null;
    decision_style: string | null; seniority_signal: string | null;
    approachability: number | null; quirks: string[]; notable_topics: string[];
    based_in: string | null; attended_events: string[];
    preferred_contact_style: string | null; reasoning: string | null;
  }>(
    `SELECT model_name, tone, professionalism, verbosity, responsiveness,
            decision_style, seniority_signal, approachability,
            quirks, notable_topics, based_in, attended_events,
            preferred_contact_style, reasoning
  ...
  const psycho = psychoRes.rows[0] ?? null;
  ...
  return {
    ...
    psychographic: psycho ? {
      model: psycho.model_name,
      tone: psycho.tone,
      professionalism: psycho.professionalism,
      verbosity: psycho.verbosity,
      responsiveness: psycho.responsiveness,
      decision_style: psycho.decision_style,
      seniority_signal: psycho.seniority_signal,
      approachability: psycho.approachability,
      quirks: psycho.quirks ?? [],
      notable_topics: psycho.notable_topics ?? [],
      based_in: psycho.based_in,
      attended_events: psycho.attended_events ?? [],
      preferred_contact_style: psycho.preferred_contact_style,
